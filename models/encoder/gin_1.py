...
class GINConv_1(MessagePassing):

    def __init__(self, nn:Callable, eps: float=0., train_eps: bool=False,
                 activation="softplus", **kwargs):
        super(GINConv_1, self).__init__(aggr='add', **kwargs)
        self.nn = nn
...
class GINEncoder_1(torch.nn.Module):
    ...
    def __init__(self, hidden_dim, num_convs=3, activation='relu', short_cut=True, concat_hidden=False, node_channels=5):
        ...
        for i in range(self.num_convs):
            self.convs.append(GINConv_1(MultiLayerPerceptron(hidden_dim, [hidden_dim, hidden_dim],\
