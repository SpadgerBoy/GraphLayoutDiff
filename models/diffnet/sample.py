...
class GraphDiffusionNetwork(nn.Module):
    ...
    def langevin_dynamics_sample_diffusion(self, node_emb, node_type, node_degree, pos_ref, fragment_mask, linker_mask, pos_init, edge_index, edge_type, batch, num_graphs, extend_order, extend_radius=True,
        ...
        with torch.no_grad():
            ...
            # pos_fragment_inital = center_pos_fragment(pos_ref, fragment_mask, batch)

            # transfer initial
            # COM_ref = scatter_mean(pos_ref[fragment_mask.bool().view(-1)], 
            #                             batch[fragment_mask.bool().view(-1)], dim=0)
            # COM_noised = scatter_mean(pos[fragment_mask.bool().view(-1)], 
            #                             batch[fragment_mask.bool().view(-1)], dim=0)
            # if COM_noised.size()[0] != 0: 
            #     pos_tranfer= pos + (COM_ref - COM_noised)[batch]
            #     pos = pos_tranfer * linker_mask + pos_ref * fragment_mask
            # else:
            #     pos = pos
            ...
            for i,j in tqdm(zip(reversed(seq), reversed(seq_next)), desc='sample'):
                for u in range(resamplings):
                    ...
                    # COM_ref = scatter_mean(pos_ref[fragment_mask.bool().view(-1)], 
                    #                             batch[fragment_mask.bool().view(-1)], dim=0)
                    # COM_noised = scatter_mean(pos_next[fragment_mask.bool().view(-1)], 
                    #                             batch[fragment_mask.bool().view(-1)], dim=0)
                    # if COM_noised.size()[0] != 0: 
                    #     pos_tranfer= pos_next + (COM_ref - COM_noised)[batch]
                    #     pos = pos_tranfer * linker_mask + pos_ref * fragment_mask
                    # else:
                    #     pos = pos
    ...
    #         # transfer initial
    #         COM_noised = scatter_mean(pos_fragment_inital[fragment_mask.bool().view(-1)], 
    #                                     batch[fragment_mask.bool().view(-1)], dim=0)
    #         COM_denoised = scatter_mean(pos[fragment_mask.bool().view(-1)], 
    #                                     batch[fragment_mask.bool().view(-1)], dim=0)
    #         if COM_denoised.size()[0] != 0: 
    #             pos_fragment_tranfer= pos_fragment_inital + (COM_denoised - COM_noised)[batch]
                
    #             pos = pos * linker_mask + pos_fragment_tranfer * fragment_mask
    #         else:
    #             pos = pos
    ...
    #                 # if i > 0:
    #                 #     eps_fragment = torch.randn_like(pos_fragment_inital)
    #                 #     pos_fragment_add_noise = alpha_cumprod[i-1].sqrt() * pos_fragment_inital + \
    #                 #         (1-alpha_cumprod[i-1]).sqrt() * eps_fragment
    #                 # else:
    #                 #     pos_fragment_add_noise = pos_fragment_inital
                    
    #                 # COM_noised = scatter_mean(pos_fragment_add_noise[fragment_mask.bool().view(-1)], 
    #                 #                             batch[fragment_mask.bool().view(-1)], dim=0)
    #                 # COM_denoised = scatter_mean(pos_next[fragment_mask.bool().view(-1)], 
    #                 #                             batch[fragment_mask.bool().view(-1)], dim=0)

    #                 # # transfer fragment to linker system
    #                 # if COM_denoised.size()[0] != 0: 
    #                 #     pos_fragment_tranfer= pos_fragment_add_noise + (COM_denoised - COM_noised)[batch]
    #                 #     pos = pos_next * linker_mask + pos_fragment_tranfer * fragment_mask
    #                 # else:
    #                 #     pos = pos_next
    ...
    def langevin_dynamics_sample_no_control(self, node_emb, node_type, node_degree, pos_ref, fragment_mask, linker_mask, pos_init, edge_index, edge_type, batch, num_graphs, extend_order, extend_radius=True,
                                n_steps=100, step_lr=0.0000010, clip=1000, clip_local=None, clip_pos=None, min_sigma=0,
                                global_start_sigma=float('inf'), w_global=0.2, w_reg=1.0, **kwargs):
        
        if self.model_type == 'diffusion':
            return self.langevin_dynamics_sample_diffusion_no_control(node_emb, node_type, node_degree, pos_ref, fragment_mask, linker_mask, pos_init, edge_index, edge_type, batch, num_graphs, extend_order, extend_radius,
                        n_steps, step_lr, clip, clip_local, clip_pos, min_sigma,
                        global_start_sigma, w_global, w_reg,
                        sampling_type=kwargs.get("sampling_type", 'ddpm_noisy'), eta=kwargs.get("eta", 1.))
        
    def langevin_dynamics_sample_diffusion_no_control(self, node_emb, node_type, node_degree, pos_ref, fragment_mask, linker_mask, pos_init, edge_index, edge_type, batch, num_graphs, extend_order, extend_radius=True,
                                 n_steps=100, step_lr=0.0000010, clip=1000, clip_local=None, clip_pos=None, min_sigma=0,
                                 global_start_sigma=float('inf'), w_global=0.2, w_reg=1.0, **kwargs):
