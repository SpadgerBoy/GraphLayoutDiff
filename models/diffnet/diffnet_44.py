...
class GraphDiffusionNetwork(nn.Module):
    ...
    def get_loss_diffusion(self, node_emb, node_type, node_degree, fragment_mask, linker_mask, pos, edge_index, edge_type, batch, num_nodes_per_graph, num_graphs,
        ...
        num_ones = int(N * 0.15)
        tmp = torch.zeros(size=(int(N), 1), device=pos.device)
        tmp[:num_ones] = 1
        fragment_mask_1 = tmp[torch.randperm(int(N))]
        linker_mask_1 = 1 - fragment_mask_1

        # COM_pos = scatter_mean(pos[fragment_mask_1.bool().view(-1)], 
        #                             batch[fragment_mask_1.bool().view(-1)], dim=0)
        # COM_pos_perturbed = scatter_mean(pos_perturbed[fragment_mask_1.bool().view(-1)], 
        #                             batch[fragment_mask_1.bool().view(-1)], dim=0)
        
        COM_pos = center_pos_fragment(pos=pos, fragment_mask=fragment_mask_1, batch=batch)
        COM_pos_perturbed  = center_pos_fragment(pos=pos_perturbed, fragment_mask=fragment_mask_1, batch=batch)
        
        pos_fragment_tranfer = pos + (COM_pos_perturbed - COM_pos)[batch]

        pos_perturbed = pos_perturbed * linker_mask_1 + pos_fragment_tranfer * fragment_mask_1
        ...
        loss_local = (pos_noise_predict * linker_mask_1  - pos_noise * linker_mask_1) ** 2
