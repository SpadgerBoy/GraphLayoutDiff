...
class EDM_Block_Global(nn.Module):
...
class GraphDiffusionNetwork(nn.Module):
    ...
    def __init__(self, config):
        ...
        for _ in range(self.config.num_edm_block):
            block = EDM_Block(self.config)
            self.global_convs.append(block)
        
        self.model_local = self.convs
        self.model_global = self.global_convs 
    ...
    def forward(self, node_emb, node_type, node_degree, pos, edge_index, edge_type, batch, time_step,
        ...
        global_edge_mask = is_global_edge(edge_type)
        ...
        for block in self.convs:
            pos_final = block(
                ...
                edge_type = edge_type[local_edge_mask],
                batch = batch,
                time_step = time_step,
                return_edges = True,
                extend_order = extend_order,
        ...
        pos_final_global = pos
        for block in self.global_convs:
            pos_final = block(
                node_emb = node_emb,
                node_type = node_type,
                node_degree = node_degree,
                pos = pos_final_global,
                edge_index = edge_index[:, global_edge_mask],
                edge_length = edge_length[global_edge_mask],
                edge_type = edge_type[global_edge_mask],
                batch = batch,
                time_step = time_step,
                return_edges = True,
                extend_order = extend_order,
        ...
        pos_noise_global = pos_final_global - pos

        return pos_noise, pos_noise_global
    ...
    def get_loss_diffusion(self, node_emb, node_type, node_degree, pos, edge_index, edge_type, batch, num_nodes_per_graph, num_graphs,
        ...
        pos_noise_predict, pos_noise_predict_global = self(
            node_emb = node_emb,
            node_type = node_type,
            node_degree = node_degree,
            pos = pos_perturbed,
        ...
        loss_global = (pos_noise_predict_global - pos_noise) ** 2
