...
class GraphDiffusionNetwork(nn.Module):
    ...
    def langevin_dynamics_sample_diffusion(self, node_emb, node_type, node_degree, pos_ref, fragment_mask, linker_mask, pos_init, edge_index, edge_type, batch, num_graphs, extend_order, extend_radius=True,
        ...
        pos_traj = []
        with torch.no_grad():
            schedule = get_repaint_schedule(resamplings=1, jump_length=1, timesteps=self.num_timesteps)
            ...
            for i, n_denoise_steps in tqdm(enumerate(schedule), desc='sample'):
                for j in range(n_denoise_steps):

                    t_array = torch.full(size=(num_graphs, ), fill_value=t, dtype=torch.long, device=pos.device)
                    ...
                    if t > 0:
                        ...
                        pos_linker = 1/alpha[t] * (pos - betas[t]/sigmas[t] * pos_noise_predict) + \
                            eps_linker * sigmas_1[t-1]/sigmas_1[t] * betas[t]
                    else:
                        pos_linker = 1/alpha[t] * (pos - betas[t]/sigmas[t] * pos_noise_predict)

                    if t > 0:
                        eps_fragment = torch.randn_like(pos_fragment_inital)
                        pos_fragment_add_noise = alpha_cumprod[t-1].sqrt() * pos_fragment_inital + \
                            (1-alpha_cumprod[t-1]).sqrt() * eps_fragment
                    else:
                        pos_fragment_add_noise = alpha_cumprod[t-1].sqrt() * pos_fragment_inital
                    ...
                    COM_denoised = scatter_mean(pos_linker[fragment_mask.bool().view(-1)], 
                                                batch[fragment_mask.bool().view(-1)], dim=0)
                    
                    # transfer fragment to linker system
                    pos_fragment_tranfer= pos_fragment_add_noise + (COM_denoised - COM_noised)[batch]

                    # combine fragment with linker
                    pos_next = pos_linker * linker_mask + pos_fragment_tranfer * fragment_mask
                    pos_next = center_pos(pos_next, batch)

                    if j == n_denoise_steps-1 and i < len(schedule) - 1:
                        pos_next = (1-betas[t]).sqrt() * pos_next + betas[t].sqrt() * torch.randn_like(pos_next)
                        pos_next = center_pos(pos_next, batch)
                        t = t + 1
