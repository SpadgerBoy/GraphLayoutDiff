...
if __name__ == '__main__':
    ...
    parser.add_argument('--logdir', type=str, default='./logs_final')
    ...
    if config.model.pe_type == "laplacian":
        transforms = Compose([
            ...
            AddUndiectedEdge(),
            AddNodeType(),
            # AddNodeMask(node_mask=config.model.node_mask),
            AddNodeDegree(),
    ...
    if config.model.regular_data:
        train_set = GraphLayoutDataset(config.dataset.train, path2=config.dataset.train_path, path3 = config.dataset.train_tree, transform=transforms)
        val_set = GraphLayoutDataset(config.dataset.val ,transform=transforms)
    else:
        train_set = GraphLayoutDataset(config.dataset.final_train, transform=transforms)
        val_set = GraphLayoutDataset(config.dataset.final_val, transform=transforms)
    ...
    def train(it):
        ...
        logger.info('[Train] Iter %05d | Loss %.2f | Loss(Global) %.2f | Loss(Local) %.2f | Grad %.2f | LR(Local) %.6f' % (
            it, loss.item(), loss_global.mean().item(), loss_local.mean().item(), orig_grad_norm, optimizer_local.param_groups[0]['lr'],
        ))
